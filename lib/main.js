...
export function renderBanner() {
        ...
        event.sponsors.forEach(sponsor => {
            html += `
                <div class="sponsor-logo">
                    <img width="140" height="140" src="${sponsor.logo_url}" alt="">
    ...
    html += `<div class="sponsor-logo">
                    <img width="227" height="227" src="/assets/swcraft-logo.png" alt="Organisé par le meetup Software Craft Strasbourg" class="logo">
                </div>`;
    html += `
            </div>
        </div>`;
...
async function exportBanner() {
    ...
    if (!captureElement) {
        alert("Le conteneur de la bannière est introuvable");
        return;
    }
    try {
        ...
        console.error("La conversion HTML vers Canvas a échoué:", err);
        alert("Erreur lors du rendu pour export.");
    }
}
...
function downloadAsPng(canvas, fileName) {
    ...
    try {
        ...
        console.error("La conversion de Canvas vers image a échoué:", e);
        alert("Erreur lors de l'export vers image.");
    }
}
